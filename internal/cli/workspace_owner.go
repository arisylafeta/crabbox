...
case "$action" in
  acquire)
    if read_state; then
      ...
      rm -rf "$root/$key.run.$state_token".*
      rm -f "$child"
      write_state
      printf RECOVERED
      exit 0
...
group_members=$(ps -eo pgid=,stat= 2>/dev/null) || exit 74
printf '%s\n' "$group_members" | awk -v pgid="$child_pgid" '$1 == pgid && $2 !~ /Z/ { found=1 } END { exit !found }' && exit 74
rm -f "$child"`
...
while :; do
	...
	if [ -f "$run_dir/payload-done" ]; then
		...
		if ! printf '%s\n' "$members" | awk -v pgid="$child_pgid" -v leader="$child_pid" -v sentinel="$sentinel_pid" '$2 == pgid && $1 != leader && $1 != sentinel && $3 !~ /Z/ { found=1 } END { exit !found }'; then printf 'done\n' >&3; exit 0; fi
	fi
	state_token=$(sed -n '2p' "$state" 2>/dev/null || true)
	state_expiry=$(sed -n '3p' "$state" 2>/dev/null || true)
	case "$state_expiry" in ''|*[!0-9]*) state_expiry=0 ;; esac
...
start="$run_dir/start"; control="$run_dir/control"; sentinel_pid_path="$run_dir/sentinel-pid"; sentinel_ready="$run_dir/sentinel-ready"
run_owner_gate() {
	if command -v flock >/dev/null 2>&1; then
...
mkfifo "$control" || exit 74
` + inputSetup + `if command -v setsid >/dev/null 2>&1; then
	(trap '' HUP; exec setsid /bin/sh -c '/bin/sh -c '"'"'trap "" HUP TERM; exec 3<>"$1" || exit 74; : >"$2" || exit 74; while IFS= read -r command <&3; do case "$command" in expire) kill -TERM 0 2>/dev/null || true; sleep 0.2; kill -KILL 0 2>/dev/null || true ;; done) exit 0 ;; *) exit 74 ;; esac; done'"'"' crabbox-sentinel "$4" "$6" & sentinel_pid=$!; printf "%s\n" "$sentinel_pid" >"$5" || exit 74; while [ ! -f "$1" ]; do kill -0 "$sentinel_pid" 2>/dev/null || exit 74; sleep 0.05; done; sh -c "$2"; code=$?; touch "$3" || exit 74; wait "$sentinel_pid"; exit "$code"' "crabbox-workspace-$token" "$start" "$payload" "$run_dir/payload-done" "$control" "$sentinel_pid_path" "$sentinel_ready"` + inputRedirect + `) &
elif command -v perl >/dev/null 2>&1; then
	(trap '' HUP; exec perl -MPOSIX -e 'POSIX::setsid() >= 0 or exit 74; exec @ARGV' /bin/sh -c '/bin/sh -c '"'"'trap "" HUP TERM; exec 3<>"$1" || exit 74; : >"$2" || exit 74; while IFS= read -r command <&3; do case "$command" in expire) kill -TERM 0 2>/dev/null || true; sleep 0.2; kill -KILL 0 2>/dev/null || true ;; done) exit 0 ;; *) exit 74 ;; esac; done'"'"' crabbox-sentinel "$4" "$6" & sentinel_pid=$!; printf "%s\n" "$sentinel_pid" >"$5" || exit 74; while [ ! -f "$1" ]; do kill -0 "$sentinel_pid" 2>/dev/null || exit 74; sleep 0.05; done; sh -c "$2"; code=$?; touch "$3" || exit 74; wait "$sentinel_pid"; exit "$code"' "crabbox-workspace-$token" "$start" "$payload" "$run_dir/payload-done" "$control" "$sentinel_pid_path" "$sentinel_ready"` + inputRedirect + `) &
else
	exit 74
fi
child_pid=$!
...
attempt=0
while [ "$child_pgid" != "$child_pid" ] && kill -0 "$child_pid" 2>/dev/null && [ "$attempt" -lt 20 ]; do sleep 0.01; attempt=$((attempt + 1)); child_pgid=$(ps -o pgid= -p "$child_pid" 2>/dev/null | tr -d ' '); done
attempt=0
...
if [ "$child_pgid" != "$child_pid" ] || [ "$sentinel_pid" -le 1 ] || [ -z "$sentinel_identity" ]; then kill "$child_pid" 2>/dev/null || true; if [ "$sentinel_pid" -gt 1 ]; then kill "$sentinel_pid" 2>/dev/null || true; kill -KILL "$sentinel_pid" 2>/dev/null || true; fi; rm -rf "$run_dir"; exit 74; fi
install_body=` + shellQuote(installBody) + `
export state child gate token child_pid child_pgid sentinel_pid sentinel_identity run_dir start control sentinel_pid_path sentinel_ready install_body
# Only the validated sentinel signals its PGID; record tamper abandons without signaling.
